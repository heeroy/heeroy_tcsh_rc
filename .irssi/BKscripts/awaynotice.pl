...
sub away_hilight_notice {
    ...
    if ($server->{usermode_away}) {
        ...
        if (!$lasthilight{lc($hilight)}{'last'}) {
      	    ...
      	    $server->command("^NOTICE $hilight [A] I am away, $server->{away_reason}. You can leave a msg or contact me later.");
            return;
         }
        ...
        if ($timeout > $ctimeout) {
            $lasthilight{lc($hilight)}{'last'} = time();
            $server->command("^NOTICE $hilight I am afk at the moment, state your request in query or contact me later, gone ($server->{away_reason})");
        }
    }
}
